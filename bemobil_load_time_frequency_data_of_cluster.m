...
if isempty(latencyMeans)
    ...
    timeIndices(2) = find(times<latencyMeans(3),1,'last');
end
...
if ~isempty(n_permutes) && ~isempty(alpha)
    ...
    for condition = 1:length(experiment_conditions_to_plot)
        ...
        [~, ~, average_time_frequency_data.(['condition_' num2str(condition)]).statistics.p_values, ~] =...
            statcond( {base_dB_all_subjects.(['condition_' num2str(condition)]) raw_dB_all_subjects.(['condition_' num2str(condition)])}, 'mode', 'bootstrap', 'naccu', n_permutes);
        
        % correct for multiple comparison using false discovery rate
        [average_time_frequency_data.(['condition_' num2str(condition)]).statistics.p_value, average_time_frequency_data.(['condition_' num2str(condition)]).statistics.p_values_mask] =...
    ...
    if length(experiment_conditions_to_plot) == 2
        ...
        [~, ~, average_time_frequency_data.difference.statistics.p_values, ~] =...
            statcond( {ersp_all_subjects.(['condition_' num2str(1)]) ersp_all_subjects.(['condition_' num2str(2)])}, 'mode', 'bootstrap', 'naccu', n_permutes);
        
        % correct for multiple comparison using false discovery rate
        [average_time_frequency_data.difference.statistics.p_value, average_time_frequency_data.difference.statistics.p_values_mask] =...
